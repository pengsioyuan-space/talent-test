...
export function Report() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const r = await fetch(`/api/report?rid=${encodeURIComponent(rid)}&_t=${Date.now()}`, {
          method: "GET",
      ...
      <div className="min-h-screen">
        ...
        <div className="mx-auto max-w-3xl px-4 py-16">
          <Card className="p-8">
            ...
            <div className="mt-2 text-slate-600">{err || "可能 rid 已过期或未生成成功。"}</div>
            <button
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-6xl px-4 -mt-8 md:-mt-10 pb-10">
          <Card className="p-6 md:p-8">
            <div className="grid md:grid-cols-2 gap-8 items-stretch">
              <div>
                ...
                <div className="mt-2 text-xs text-slate-500">分数区间：0-20（越高越突出）</div>
              </div>
                  ...
                  {dimsSorted.map((d) => {
                    ...
                    return (
                      <div key={d.dim} className="flex items-center gap-3">
                        ...
                        <div className="w-10 sm:w-12 text-sm text-slate-700 text-right">{d.score20}</div>
                        <div className="hidden sm:block w-20 text-xs text-slate-500 text-right">{level}</div>
                      </div>
                    );
                  })}
                </div>
      ...
      <div className="mx-auto max-w-6xl px-4 pb-16">
        <div className="grid lg:grid-cols-3 gap-6">
          <Card className="p-6">
            ...
            <div className="mt-4 text-slate-600 leading-relaxed">{narrative?.summary}</div>
            <div className="mt-5 flex flex-wrap gap-2">
              {top3.map((t, i) => (
                <span
                  key={`${t}-${i}`}
                  className={["px-3 py-1 rounded-full text-xs font-medium", badgeColor(i)].join(" ")}
                >
                  {t}
                </span>
              ))}
